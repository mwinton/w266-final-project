...
class VQADataset:
    ...
    def batch_generator(self, batch_size):
        ...
        image_cache_start = 0
        image_cache_size = batch_size * 100

        while True:

            ## load cache size images into memory whenever the batch requires it
            ## This is still at the mercy of the garbage collector to reuse the memory
            if (batch_end > image_cache_start ): 
                if (image_cache_start != 0):
                    # previous sample to image bindings need to be reset.
                    for idx in range(image_cache_start,image_cache_end):
